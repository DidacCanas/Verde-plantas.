...
function renderModal(){ const d=selectedData(); $('#diagnosisIcon').textContent=d.icon; $('#resultTitle').textContent=d.name; $('#confidence').textContent=d.confidence; $('#diagnosisCopy').textContent=d.copy; $('#checkFirst').textContent=d.check; $('#timeline').innerHTML=d.steps.map(s=>`<li><span class="day">${s[0]}</span><div class="step"><b>${s[1]}</b><p>${s[2]}</p></div></li>`).join(''); $('#preventionList').innerHTML=d.prevent.map(p=>`<li>${p}</li>`).join(''); }
...
function updateIdentifyButton(){ $('#identifySpeciesBtn').disabled = !(imageFile && keyInput.value.trim()); }
function plantNameFromResult(result){ const species = result.species || {}; const scientific = species.scientificNameWithoutAuthor || species.scientificName || 'Especie sin nombre disponible'; const common = Array.isArray(species.commonNames) && species.commonNames[0] ? species.commonNames[0] : ''; return {scientific, common}; }
function setDetectedPlant(name){ const select=$('#plant'); let option=[...select.options].find(o=>o.text===name); if(!option){ option=document.createElement('option');option.value=`detect:${name}`;option.text=name;select.add(option,0); } select.value=option.value; }
$('#identifySpeciesBtn').addEventListener('click', async () => {
  ...
  button.disabled=true;button.querySelector('span').textContent='Identificando especie…';status.textContent='Enviando la foto a Pl@ntNet para analizarla.';
  try{
    const data=new FormData();data.append('images',imageFile,imageFile.name);data.append('organs','auto');
    ...
    const plant=plantNameFromResult(top); const confidence=Math.round((top.score||0)*100);
  ...
  }finally{button.querySelector('span').textContent='Identificar especie en la foto';updateIdentifyButton();}
});
$('#analyzeBtn').addEventListener('click',()=>{renderModal();open('resultModal')});
document.querySelectorAll('[data-close]').forEach(b=>b.addEventListener('click',()=>close(b.dataset.close)));
document.querySelectorAll('.modal-backdrop').forEach(m=>m.addEventListener('click',e=>{if(e.target===m)close(m.id)}));
$('#historyBtn').addEventListener('click',()=>open('historyModal')); $('#openHistory').addEventListener('click',()=>open('historyModal'));
...
$('#savePlanBtn').addEventListener('click',()=>{const d=selectedData();const p={id:Date.now(),name:d.name,plant:$('#plant').options[$('#plant').selectedIndex].text,date:new Date().toLocaleDateString('es-ES'),steps:d.steps,done:[]};const plans=getPlans();plans.unshift(p);setPlans(plans);close('resultModal');renderTasks();$('#todaySection').scrollIntoView({behavior:'smooth'});});
function renderTasks(){const plans=getPlans(), list=$('#taskList'), empty=$('#emptyState'); const tasks=plans.flatMap(p=>p.steps.map((s,i)=>({...p,s,i}))); const undone=tasks.filter(t=>!t.p.done.includes(t.i)); $('#todayCount').textContent=`${undone.length} ${undone.length===1?'tarea':'tareas'}`;empty.hidden=plans.length>0;list.innerHTML=undone.map(t=>`<article class="task"><button class="task-check" data-id="${t.p.id}" data-step="${t.i}" aria-label="Marcar como realizada"></button><div class="task-copy"><b>${t.s[1]}</b><span>${t.p.plant} · ${t.p.name}</span></div><span class="task-day">${t.s[0]}</span></article>`).join(''); document.querySelectorAll('.task-check').forEach(b=>b.addEventListener('click',()=>{let all=getPlans();let p=all.find(x=>x.id==b.dataset.id);let i=+b.dataset.step;p.done.includes(i)?p.done=p.done.filter(x=>x!==i):p.done.push(i);setPlans(all);renderTasks()})); $('#historyDot').classList.toggle('show',plans.length>0);}
function renderHistory(){const p=getPlans(), h=$('#historyList');h.innerHTML=p.length?p.map(x=>`<div class="history-item"><div class="history-icon">⌁</div><div><b>${x.name}</b><span>${x.plant} · guardado el ${x.date} · ${x.done.length}/${x.steps.length} tareas</span></div></div>`).join(''):'<p class="no-history">Todavía no has guardado ninguna revisión.</p>'}
renderTasks();
